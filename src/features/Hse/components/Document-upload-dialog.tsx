...
const documentFormSchema = z.object({
  ...
  docs_types: z.string().min(1, 'Debe seleccionar un tipo de documento'),
    ...
    .refine(
      (file) => {
        ...
        if (file instanceof File) {
          return ['.pdf', '.doc', '.docx'].some((ext) => file.name.toLowerCase().endsWith(ext));
        }
...
export function DocumentUploadDialog({
  ...
  const onSubmit = async (data: DocumentFormValues) => {
    try {
      ...
      const message = error instanceof Error ? error.message : 'Hubo un problema al guardar el documento';

      toast({
        title: message.includes('ya existe') ? 'El documento ya existe' : 'Error al guardar el documento',
        description: message,
        variant: 'destructive',
        duration: 5000,
      });
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          <DialogTitle>{mode === 'edit' ? 'Editar Documento' : 'Subir Nuevo Documento'}</DialogTitle>
          <DialogDescription>
            {mode === 'edit' ? 'Modifica los datos del documento' : 'Agrega un nuevo documento al sistema'}
          </DialogDescription>
        </DialogHeader>
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
            ...
            <FormField
              ...
              render={({ field, fieldState }) => {
                const selectedValues = Array.isArray(field.value) ? field.value.filter(Boolean) : [];

                const validSelectedValues = selectedValues.filter((id) => allTags.some((tag) => tag.id === id));
                    ...
                    <div className="mb-2 p-2 bg-gray-50 rounded-md">
                      ...
                      <p className="text-xs text-muted-foreground">
                        ...
                        {form.getValues('file_size')
                          ? `${Math.round(Number(form.getValues('file_size')) / 1024)} KB`
                          : 'N/A'}
                      </p>
                      <p className="text-xs text-muted-foreground">Si subes un nuevo archivo, reemplazará al actual.</p>
                    </div>
                  )}
                  <FormControl>
