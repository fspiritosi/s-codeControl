...
import { BookOpen, Briefcase, Calculator, Users, Truck, Fuel, TrendingUp, Layers, ListChecks } from 'lucide-react';

async function getCostosCounts(companyId: string) {
  const [cct, equiposConCosto, servicios, composiciones, formulas, tiposConCosto, conceptos] = await Promise.all([
    ...
    prisma.concepto_equipo.count({ where: { company_id: companyId } }),
  ]);
  return { cct, equiposConCosto, servicios, composiciones, formulas, tiposConCosto, conceptos, liquidaciones: 0 };
}
...
export default async function CostosDashboard() {
  ...
  const counts = companyId
    ...
    : { cct: 0, equiposConCosto: 0, servicios: 0, composiciones: 0, formulas: 0, tiposConCosto: 0, conceptos: 0, liquidaciones: 0 };

  const cards = [
    { title: 'CCTs configurados',  value: counts.cct,             icon: BookOpen,    href: '/dashboard/costos/configuracion-cct' },
    { title: 'Conceptos de costo', value: counts.conceptos,       icon: ListChecks,  href: '/dashboard/costos/conceptos' },
    ...
    { title: 'Fórmula polinómica', value: counts.formulas,        icon: TrendingUp,  href: '/dashboard/costos/formula-polinomica' },
