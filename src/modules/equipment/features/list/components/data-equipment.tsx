...
export function EquipmentTable<TData, TValue>({ columns, data, role }: DataEquipmentProps<TData, TValue>) {
  // Identificador unico para esta tabla
  ...
  // Anadir estado para el tamano de pagina
  ...
  // Configuracion de la tabla con filtrado personalizado para status
  const table = useReactTable({
    data,
    columns: defaultColumns.map((col) => ({
      ...col,
      id: col.accessorKey || col.id,
      // Anadir filtrado personalizado para la columna status
      ...(col.accessorKey === "status" && {
        filterFn: (row, columnId, filterValue) => {
          const value = row.getValue(columnId)
          // Si no hay valor de filtro, mostrar todas las filas
          if (!filterValue) return true
          // Comparacion exacta para evitar que "Completo" coincida con "Incompleto"
    ...
    // Anadir manejador para cambios en la paginacion
  ...
  // Aplicar los filtros guardados cuando la tabla este lista
  useEffect(() => {
  ...
  return (
    <div className="w-full grid grid-cols-1">
      <div className="flex items-center py-4 flex-wrap gap-y-2 overflow-auto">
        <Input
          placeholder="Buscar por Dominio o Numero de interno"
                            ...
                            header.id in selectHeader ? (
                              header.id === "allocated_to" ? (
                                <div className="flex justify-center">
                                  <Input
                                    placeholder="Buscar por afectacion"
