...
import { z } from 'zod';
import { reactivateCovenant, deactivateCovenantAndCategories, updateCovenantById } from '@/modules/company/features/covenants/actions.server';
import { fetchCovenantsByCompany } from '@/modules/company/features/covenants/actions.server';
...
export const columns: ColumnDef<Colum>[] = [
  {
    ...
    cell: ({ row }: { row: any }) => {
      ...
      const fetchCovenant = async () => {
        try {
          await fetchCovenantsByCompany(actualCompany?.id || '');
        } catch (error) {
