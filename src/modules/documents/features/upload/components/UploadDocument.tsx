...
import { YearMonthPicker } from '@/shared/components/ui/year-month-picker';
...
const ENTITY_CONFIG = {
  employee: {
    tableName: 'documents_employees' as const,
    resourceLabel: 'Empleado',
    resourceLabelLower: 'empleado',
    searchPlaceholder: 'Buscar empleado',
    selectPlaceholder: 'Seleccionar empleado',
    selectDescription: 'Selecciona el empleado al que le corresponde el documento',
    documentTypeDescription: 'Seleccione el tipo de documento que desea cargar al empleado',
    documentCategory: 'persona' as const,
  },
  equipment: {
    tableName: 'documents_equipment' as const,
    resourceLabel: 'Equipo',
    resourceLabelLower: 'equipo',
    searchPlaceholder: 'Buscar equipo',
    selectPlaceholder: 'Seleccionar equipo',
    selectDescription: 'Selecciona el equipo al que le corresponde el documento',
    documentTypeDescription: 'Seleccione el tipo de documento que desea cargar al equipo',
    documentCategory: 'equipos' as const,
  },
};
...
function UploadDocument({
  ...
  const [selectedResourceDocuments, setSelectedResourceDocuments] = useState<
    Database['public']['Tables']['documents_employees']['Row'][] | Database['public']['Tables']['documents_equipment']['Row'][]
  >([]);
  ...
  useEffect(() => {
    fetchSelectedResourceDocuments();
  }, [default_id]);

  const getAppliesLabel = (resourceItem: ResourceItem): string => {
    if (entityType === 'employee') {
      return resourceItem.label;
    }
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem className="flex flex-col">
                <FormLabel>{config.resourceLabel}</FormLabel>
                <Popover>
                  <PopoverTrigger asChild>
                    <FormControl>
                      ...
                      >
                        {field.value
                          ? resources.find((r) => r.value === field.value)?.label
                          ...
                          {resources.map((resourceItem) => (
                            ...
                            >
                              <Check
                                className={cn(
                                  'mr-2 h-4 w-4',
                                  resourceItem.value === field.value ? 'opacity-100' : 'opacity-0'
                                )}
                              />
                              {resourceItem.cuit
                                ? resourceItem.label + ' - ' + resourceItem.cuit
                                : resourceItem.label}
                            </CommandItem>
                          ))}
                        </CommandGroup>
                ...
                <FormDescription>{config.selectDescription}</FormDescription>
                <FormMessage />
              </FormItem>
            )}
          />
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem className="flex flex-col">
                          ...
                          {documenTypes.map((documentType) => (
                            <CommandItem
                              ...
                              disabled={
                                ...
                                (selectedResourceDocuments as any[]).find(
                ...
                <FormDescription>{config.documentTypeDescription}</FormDescription>
                <FormMessage />
              </FormItem>
            )}
          />
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem className="flex flex-col">
                  ...
                  <FormDescription>Seleccione el periodo del documento</FormDescription>
                  <FormMessage />
                </FormItem>
              )}
            />
                ...
                <FormControl>
                  <div className="flex items-center space-x-2">
                    <Input
                      type="text"
                      readOnly
                      value={selectedFileName || field.value || 'Ningun archivo seleccionado'}
                    ...
                    <Button
                      ...
                      onClick={() => {
                        ...
                        input.onchange = async (e) => {
                          ...
                          if (file) {
                            const applies = getAppliesLabel(
                              resources.find((r) => r.value === form.getValues('applies'))!
                            );
