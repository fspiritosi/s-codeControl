...
const UploadDocument: React.FC<UploadDocumentProps> = ({ rowId, companyName, customerName, serviceName, itemNames, isReplacing }) => {
    const supabase = supabaseBrowser();
    const [isDialogOpen, setIsDialogOpen] = useState(false);
    const [selectedFile, setSelectedFile] = useState<File | null>(null);
    const route = useRouter();
    ...
    const handleUpload = async () => {
        ...
        if (isReplacing) {
            ...
            const currentFilePathModified = currentData.document_path ? currentData.document_path.split('/').slice(1).join('/') : '';
            if (!currentFilePath) {
