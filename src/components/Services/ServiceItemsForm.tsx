...
const EditItemSchema = z.object({
    ...
    item_price: z.preprocess((val) => Number(val), z.number().min(0, { message: "Debe ingresar un precio válido" }).optional()),
...
export default function ServiceItemsForm({ measure_units, customers, services, company_id, editingService }: { measure_units: measure_unit[], customers: customer[], services: Service[], company_id: string, editingService: Item }) {
    ...
    return (
        <Form {...form}>
            <form onSubmit={form.handleSubmit(handleSubmit1)} className="space-y-8">
                ...
                />
                <Button className="mt-4" type="submit">{isEditing ? 'Editar' : 'Cargar'}</Button>
                <Button className="mt-4 ml-2" type="button" onClick={handleCancel}>Cancelar</Button>
                {isEditing && (
                    <Button className="mt-4 ml-2" onClick={handleDeactivateItem} variant={editingService.is_active ? 'destructive' : 'success'}>
