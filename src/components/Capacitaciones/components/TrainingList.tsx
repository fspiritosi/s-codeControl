import { GraduationCap, Users, Trash2, Eye, Tag as TagIcon } from 'lucide-react';
            ...
            <div className="flex items-center gap-2 flex-wrap truncate">
              ...
              {training.tags.length > 0 && (
                <div className="flex gap-1 flex-wrap">
                  ...
                  {training.tags.length > 2 && training.tags[0]?.color && (
                    <Badge 
                      variant="outline" 
                      className="text-xs"
                      style={{
                        backgroundColor: `${training.tags[0].color}20`,
                        borderColor: `${training.tags[0].color}40`,
                        color: training.tags[0].color
                      }}
                    >
                      +{training.tags.length - 2} más
                    </Badge>
                  )}
                </div>
              )}
            ...
            <div className="flex items-center gap-4 text-xs text-muted-foreground mt-0.5">
              ...
              <div className="flex items-center gap-1">
                {training.materials.length > 0 && (
                  <span>{training.materials.length} material{training.materials.length !== 1 ? 'es' : ''}</span>
                )}
                
                {training.evaluation?.questions && training.evaluation.questions.length > 0 ? (
                  <>
                    <span>•</span>
                    <span>{training.evaluation.questions.length} pregunta{training.evaluation.questions.length !== 1 ? 's' : ''}</span>
          ...
          <div className="flex items-center gap-2">
            <div className="flex items-center gap-1">
              ...
              <Button
                ...
                size="sm"
                className="h-8"
                onClick={() => onViewEmployee(training.id)}
              >
