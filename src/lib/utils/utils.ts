...
export const formatTableDocument = (doc: DocumentEmployeesWithRelations) => {
  return {
    date: new Date(doc.created_at).toLocaleDateString(),
...
export const formatTableDocumentVehicle = (doc: DocumentEquipmentWithRelations) => {
  return {
...
export const FormatEmployeeRelationData = (employees: EmployeeWithRelations[]) => {
...
export const FormatVehiclesToShow = (vehicles: VehiclestWithRelations[]) => {
  return vehicles?.map((item) => ({
    ...item,
